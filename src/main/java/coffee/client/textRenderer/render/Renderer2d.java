package coffee.client.textRenderer.render;

import com.mojang.blaze3d.systems.RenderSystem;
import coffee.client.textRenderer.util.BufferUtils;
import coffee.client.textRenderer.util.Colors;
...
import static coffee.client.textRenderer.render.Renderer3d.getColor;
import static coffee.client.textRenderer.util.RendererUtils.endRender;
import static coffee.client.textRenderer.util.RendererUtils.setupRender;
...
public class Renderer2d {
    ...
    public static void renderRoundedQuad(MatrixStack matrices, Color c, double fromX, double fromY, double toX, double toY, float radTL, float radTR, float radBL, float radBR, float samples) {
        ...
        renderRoundedQuadInternal(matrix, r, g, b, a, (float) fromX, (float) fromY, (float) toX, (float) toY, radTL, radTR, radBL, radBR, samples);
        endRender();
    }
    ...
    private static void renderRoundedOutlineInternal(Matrix4f matrix, float cr, float cg, float cb, float ca, float fromX, float fromY, float toX, float toY, float radC1, float radC2, float radC3, float radC4, float width, float samples) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (float r = i * 90f; r <= (i + 1) * 90f; r += (90 / samples)) {
                ...
                float cos = cos1 * rad;
                bufferBuilder.vertex(matrix, current[0] + sin, current[1] + cos, 0.0F).color(cr, cg, cb, ca).next();
                bufferBuilder.vertex(matrix, current[0] + sin + sin1 * width, current[1] + cos + cos1 * width, 0.0F).color(cr, cg, cb, ca).next();
            }
        }
    ...
    public static void renderRoundedOutline(MatrixStack matrices, Color c, double fromX, double fromY, double toX, double toY, float radTL, float radTR, float radBL, float radBR, float outlineWidth, float samples) {
        ...
        renderRoundedOutlineInternal(matrix, r, g, b, a, (float) fromX, (float) fromY, (float) toX, (float) toY, radTL, radTR, radBL, radBR, outlineWidth, samples);
        endRender();
    }
